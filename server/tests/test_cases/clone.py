...
class TestClone(helpers.TestCase):
  ...
  def test_clone_annotatation(self):
    ...
    self.assertEqual(info[0], {'D': '3', 'format': 'CHROMOSOME,START,END', 'extra_metadata': {'new': 'true'}, 'upload_info': {'upload_end': '0', 'done': 'true', 'cloned_from': 'a3'}, 'name': '', 'genome': 'hg19', '_id': 'a4', 'type': 'annotation', 'columns': [{'name': 'CHROMOSOME', 'column_type': 'string'}, {'name': 'START', 'column_type': 'integer'}, {'name': 'END', 'column_type': 'integer'}], 'description': 'New CpG Islands'})
