...
class TestClone(helpers.TestCase):
  ...
  def test_clone_annotatation(self):
    ...
    self.assertEqual(info[0], {'format': 'CHROMOSOME,START,END', 'extra_metadata': {'new': 'true'}, 'upload_info': {'client_address': '127.0.0.1', 'total_size': '0', 'content_format': 'peaks', 'upload_start': '0', 'upload_end': '0', 'done': 'true', 'user': 'test_admin'}, 'name': 'New CpG Islands', 'genome': 'hg19', '_id': 'a4', 'type': 'annotation', 'columns': [{'name': 'CHROMOSOME', 'column_type': 'string'}, {'name': 'START', 'column_type': 'integer'}, {'name': 'END', 'column_type': 'integer'}], 'description': 'CpG islands are associated ...'})
