...
class TestBioSourceCommands(helpers.TestCase):
  ...
  def test_bio_source_scope(self):
    epidb = EpidbClient()
    self.init(epidb)
  ...
  def test_check_embracing_bug(self):
	...
	res = epidb.add_bio_source("A", None, {}, self.admin_key)
	self.assertSuccess(res)

	res = epidb.set_bio_source_synonym("A", "B", self.admin_key)
	self.assertSuccess(res)

	res = epidb.add_bio_source("B", None, {}, self.admin_key)
	self.assertFailure(res)

	res = epidb.add_bio_source("C", None, {}, self.admin_key)
	self.assertSuccess(res)

	res = epidb.set_bio_source_scope("B", "C", self.admin_key)
	self.assertSuccess(res)

	res = epidb.get_bio_source_related("A", self.admin_key)
	self.assertSuccess(res)
	self.assertEqual(res[1], [['bs1', 'A'], ['bs1', 'B'], ['bs2', 'C']])

	res = epidb.get_bio_source_related("C", self.admin_key)
	self.assertSuccess(res)
