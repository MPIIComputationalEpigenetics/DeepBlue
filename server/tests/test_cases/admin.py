...
class TestAdminCommands(helpers.TestCase):
  ...
  def test_echo(self):
    epidb = EpidbClient()
    self.init(epidb)
    self.assertEquals(epidb.echo(self.admin_key), ['okay', 'DeepBlue (1.1.0) says hi to test_admin'])
    self.assertEquals(epidb.echo("invalid"), ['okay', 'DeepBlue (1.1.0) says hi to a Stranger'])
