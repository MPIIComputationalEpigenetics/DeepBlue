...
def get_chromatin_stes(genome, uk):
    ...
    status, csss_query_id = server.select_regions(
        None, genome, "Chromatin State Segmentation", None, None, None, "chr21", None, None, uk)
...
def build_chromatin_state_files(uk):
    ...
    for css_file in css_files_names:
        ...
        ss, css_file_query_cache_id = server.query_cache(
            css_file_query_id, True, uk)
...
server = xmlrpclib.Server(
    "http://deepblue.mpi-inf.mpg.de/xmlrpc", allow_none=True)
...
s, request = server.enrich_region_overlap(query_id, universe_query_id, datasets, "grch38", uk)
print request
