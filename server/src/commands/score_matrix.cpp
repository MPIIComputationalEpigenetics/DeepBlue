  ...
  namespace command {
    ...
    public:
                       ...
                       const serialize::Parameters &parameters, serialize::Parameters &result) const
      {
        ...
        std::vector<std::string> norm_experiments_names;
        ...
        for (auto &experiment_name : experiments) {
          ...
          norm_experiments_names.push_back(experiment["norm_name"].String());
        ...
        for (auto &chromosome : range_regions) {
          ...
          for (auto &region : *chromosome.second) {
            ...
            for (auto &experiment_name : norm_experiments_names) {
              mongo::BSONObj regions_query;
              if (!dba::query::build_experiment_query(region.start(), region.end(), experiment_name, user_key, regions_query, msg)) {
                result.add_error(msg);
                return false;
              }
